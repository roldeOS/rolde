...
| ▢ | **Scribe T2.5–T4** — T2.5 autotext shortcuts (Carebit steal) → T3 Caretaker clinic templates + clinic-named colour legends (approved 2026-07-13) → T4 intake/consent + payment hook + Phase-B form steals |
...
*Last updated: 2026-07-13 (Roland back from India — his father's surgery week. Popover Anatomy LOCKED in URDS + audit clean; FACE PORTRAIT v7 [own artwork, canon proportions, hair over shoulders — screenshot-proven]; embedded map FIXED FOR REAL [explicit px heights — h-full % was collapsing in Safari; harness-screenshot proof]; SCRIBE T2 SHIPPED [personal template builder + snapshot law + owner-only RLS, E2E + hostile-payload verified]; T3 scope += clinic-named colour legends [approved]; Phases A+B+C swept [C finds: W5-drafts-into-templates · questionnaire analytics · wearable metrics · programme builder]; unread settled; next: T2.5 autotext → Phase D on Roland's word → T3.)*
