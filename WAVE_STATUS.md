...
*Last updated: 2026-07-21 v4 (**B1+B2+B3+B4 SHIPPED & verified in hydrated WebKit** — icon-only Search/Layouts (patient name unclipped) · toned chip icons · the COURIER DOOR in Scribe (form rail + letter rail, feed button retired) · Zoom v2 (pinch/scroll/double-tap, tool deleted; 12 checks × 2 engines). Bonus root-cause: feed tile stamps failed hydration on ICU variance → deterministic lib/dates + suppressHydrationWarning on local-clock spans. **✅ Phase E SWEPT** (Heidi compliance spine · Lyrebird edit-learning · Nabla SMART-on-FHIR + RCT culture · Abridge LINKED EVIDENCE · NHSE AVT: Class 1 summariser, DCB0160 pack per clinic) — W5 build spine locked in Bible 4.8 §15.7. NEXT: **B6 rich-text Scribe** (constrained editor, B/I/U/highlight, bubble+rail) as its own chunk → then Body-Map 2.2 (the Photo Spine). B5 squircle parked (Roland). Phase F on his word.)*
