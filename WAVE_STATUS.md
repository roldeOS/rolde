...
*Last updated: 2026-07-21 v7 (**Scribe polish SHIPPED & verified**: Format chip FIRST, then Templates·Anatomy·Snips·Courier; header chips collapse to icons when the card is narrow (ResizeObserver, no wrap) and show labels when wide; the floating bubble hides when the Format panel opens; symmetric spacing around Save·Discard. Universal Jarvis brief updated with the **build-patiently-then-try-to-crash-it iron-clad ritual** (Roland 2026-07-21, applies to every build). **NEXT: Photo tool Milestone 1** — the plumbing + capture (UK storage bucket · patient_photo table + RLS + audit · +Add Photos camera/upload · shrink pipeline · square thumbnails in the note), built with the full patience+crash-test ritual. 3 milestones total: (1) plumbing+capture, (2) gallery + before/after compare, (3) patient-facing view + Custodian Capacity page.)*
