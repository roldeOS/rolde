...
*Last updated: 2026-07-22 v8 (**Photo tool Milestone 1 SHIPPED & crash-tested**: private Supabase bucket + patient_photo (RLS tenant-scoped, soft-delete, audit); client-side JPEG shrink (≤2048 master + square 512 thumb, EXIF-safe); '+ Add Photos' in the consult (camera/upload · before/after tag · square tiles · tap-to-view); signed-URL reads. Crash-tests all pass: happy path (shrink→store→DB→thumbnail 200), non-image + corrupt rejected gracefully, CROSS-TENANT blocked (owner sees 1, other sees 0); found+fixed the WebKit WebP-encode gap (→JPEG). Scribe also polished: instant hover tooltips on the icon-collapsed header + tighter Format panel. NEXT: **Photo M2** — gallery + before/after compare (slider + ghost overlay); then **M3** — patient-facing view + Custodian Capacity page (→ Cloudflare R2 at scale).)*
