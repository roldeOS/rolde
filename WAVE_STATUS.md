...
*Last updated: 2026-07-21 v6 (**Format panel + editor fixes SHIPPED & verified in real WebKit**: one 'Format' header chip → full palette popover (B/I/U/**S** · 5 highlight colours · bullets/numbered/indent/outdent · clear); quick bubble stays, now portaled so it can't clip; note text scrolls (Save no longer traps the caret); Scribe clears after save. Header renames: Body Map→**Anatomy**, Snippets→**Snips** (Courier unchanged). Two bugs from Roland fixed. Git author scrubbed to roldeOS (gmail off this session's commits). **NEXT (Roland's go): the Photo tool + Custodian Capacity page** — real infrastructure, built in verified milestones: (1) storage bucket + patient_photo table + upload/shrink pipeline, (2) gallery + before/after compare, (3) patient-facing view + capacity gauges. Square display tiles, full image kept, UK region.)*
