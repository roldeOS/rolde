...
| ✅ | **B6 Rich free-note editor** (2026-07-21) — bold/italic/underline/**highlight** via a floating selection bubble + slim toolbar + ⌘B/I/U/⇧H; the SIDECAR-MARKS model (canonical text stays plain, formatting is reversible metadata; existing notes untouched); autotext + self-continuing lists + snippets run through it; `<br>`+filler newline model. Verified in real WebKit + the running app (type→bold→highlight→save→feed renders `<strong>/<mark>`; DB payload.format_marks with clean text; lists/autotext/offset-stability/toggle-off/multi-line all pass) |
| ✅ | **B2 Courier door** (2026-07-21) — one branded amber menu in the Scribe header: Send A Form (library rail) · Send A Letter (authored rail, live unsent count); feed "Send A Form" retired; photos/aftercare rows join with their engines. E2E both rails + both send sheets |
| ✅ | **B4 Body-Map Zoom v2** (2026-07-21) — pinch/scroll/double-tap/drag-pan, the Zoom TOOL deleted; 12 checks × real WebKit + Chromium |
| ✅ | **Body-Map draw colours + labels** (2026-07-21, v2.2 groundwork) — the Draw tool gets the full 8-tone palette; each stroke renders in its colour + gets a labelled rail row, reading into the record like a pin; records law holds for legacy bare strokes |
| ✅ | **B1/B3/B7 topbar + chips** (2026-07-21) — icon-only Search + Layouts (patient name unclipped); date/time · Search · Layouts order (search by the seconds); the four Scribe chips wear Earth & Bloom icon tones |
| ✅ | **Hydration fix** (2026-07-21) — feed tile stamps failed React hydration on ICU/`toLocale*` variance ("at" vs ","); deterministic `lib/dates` + `suppressHydrationWarning` on the genuinely-local-clock spans |
...
*Last updated: 2026-07-22 v9 (**Photo tool Milestone 2 SHIPPED & crash-tested**: the camera moved to a header chip next to Templates (Roland — the composer strip was wrong); captures STAGE and attach to the Clinical Note on Save; the note tile shows a labelled BEFORE/AFTER square-thumbnail gallery + a tap-to-open drag-slider COMPARE; photos-only notes allowed; discard soft-deletes staged. **HEIC** built (bundled heic2any, loads only when a non-Safari browser needs it). Crash-tests pass: stage→save→attached+rendered, compare slider clips right, discard soft-deletes, cross-tenant already proven in M1. Known hardening: orphan sweep for photos staged-then-navigated-away. NEXT: **M3** — patient-facing photo view + the Custodian Capacity page (storage/bandwidth gauges → the cue to move object storage to Cloudflare R2 at scale) + the orphan sweep.)*
