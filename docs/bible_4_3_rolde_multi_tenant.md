...
>
> The implementation specification for the Custodian/Steward authority model, tenant onboarding, billing, and per-tenant configuration. Inherits from Bible 0 v1.2, Bible 4.0, Bible 4.1, and Bible 4.2.
...
- The Steward admin panel (where each clinic configures their RolDe instance)
...
4. Bible 4.2 — design system (Steward admin uses Section 9 component patterns)
...
Step 8: Configure clinic basics (logo, tagline, primary colour, hours)
        ↓
Step 9: Add first additional users (optional — can defer to Steward admin)
        ↓
Step 10: Confirm + activate
        ↓
Tenant is provisioned, subdomain DNS resolves, Steward is logged in to their dashboard
```
...
Stripe handles all payment processing for RolDe-Ltd-to-tenant billing. Tenant-to-patient billing is a separate concern (Stripe Connect, configured per tenant in Steward admin — §5).
...
| Invoice | Invoice | Generated by Stripe; downloadable from Steward admin |
...
| `customer.subscription.trial_will_end` | Send 7-day reminder email; surface notice in Steward admin |
| `invoice.payment_succeeded` | Audit log entry; receipt email |
| `invoice.payment_failed` | `tenants.subscription_status` = 'past_due'; notify Steward; grace period 14 days |
| `payment_method.attached` | Update card-on-file in Steward admin |
| `payment_method.detached` | Notify Steward if last payment method removed |
...
- Day 0 (failure): Tenant remains active; Steward notified; Stripe attempts retry
- Day 3: Second notification; Steward dashboard shows persistent banner
- Day 7: Third notification; non-Steward users see banner on login
- Day 14: Tenant suspended (status → 'suspended'); login disabled for non-Steward roles; Steward can still access to update payment method
...
Steward can change plan at any time:
...
In Steward admin → Billing → Invoices:
...
In Steward admin → Settings → Payments:
...
Where each Steward configures and runs their clinic's RolDe instance.

### 5.1 The Admin Panel URL and Routing

Accessed at `<subdomain>.rolde.app/admin` (only visible to users with Steward role; other roles get 404).
...
Recent activity:
  ...
  - Receptionist registered new patient: Sarah Jones
...
─────────────────────────────────────────────────────────────
Roland M Jayasekhar   Steward           Active       Now
Sarah Davies          Practitioner      Active       2h ago
James Chen            Practitioner      Pending      Invited 2 days ago
Lisa Wong             Receptionist      Active       1h ago
...
Click any user row → user detail page where Steward can:
...
- **Notifications**: which events Steward gets notified about (referral failures, payment failures, AI errors)
...
Each row's destination configurable. RolDe-network connections show the receiving clinic name and Steward acceptance status (Bible 0 §12.5).
...
5. Steward of the affected tenant receives a notification: *"A RolDe Ltd staff member accessed your clinic's data on [date]. Reason: [reason]. View full audit."*
  ...
  USING (
    ...
    AND EXISTS (
      ...
      WHERE tenant_users.user_id = auth.uid()
        ...
        AND tenant_users.role IN ('steward', 'practitioner', 'locum', 'nurse')
...
export async function canIssuePrescription(userId: string, tenantId: string): Promise<boolean> {
  ...
  return (
    ['steward', 'practitioner', 'locum'].includes(user.role) &&
...
describe('Permission: canIssuePrescription', () => {
  it('returns true for Practitioner with GMC + rights', async () => { /* ... */ });
  it('returns false for Practitioner without prescribing rights', async () => { /* ... */ });
  it('returns false for Practitioner without GMC number', async () => { /* ... */ });
  it('returns false for Nurse role even with GMC', async () => { /* ... */ });
  it('returns false for Receptionist always', async () => { /* ... */ });
...
- Notification sent to all Stewards
- Steward approves or denies in their admin
...
### 8.3 The Steward Absence Management

In Steward admin → Users → Absences:
...
export async function updateTenantConfig(
  ...
  if (!await isSteward(userId, tenantId)) {
    throw new Error('Forbidden: Steward role required');
  }
...
|---|---|---|
| (none) → pending | Onboarding completed | Subdomain provisioned; Steward can log in but cannot perform clinical work yet |
...
| active → suspended | Billing past-due 14 days OR critical ToS violation | Login disabled for non-Steward roles; Steward can update payment method; clinical data retained |
...
| active → archived | Voluntary cancellation by Steward + 30 day grace | Same as above |
...
- Steward can request data export (full GDPR export) within 30 days of archive
...
A Steward can change their tenant's subdomain (e.g. rebranding):
...
1. Steward enters custom domain in tenant settings
...
- Searchable via Steward admin (own tenant) and Custodian admin (cross-tenant)

### 15.3 The Compliance Reporting

Steward can generate compliance reports from audit log:
...
- Includes all clinical data the patient is authorised to see (per Steward visibility settings)
...
- [ ] Tenant activation completes end-to-end with status update, DNS resolution, Steward login
- [ ] Abandoned wizard recovery emails send at 1, 3, 7 days

### 17.2 The Steward Admin Acceptance
...
- [ ] Custodian access notifications sent to affected Stewards
...
- [ ] Invoice display in Steward admin functional
...
- [ ] Each config field has UI in Steward admin
...
- [ ] At least one tenant configuration change made by Steward, propagated to product
...
This is the foundation everything else sits on. Onboarding, billing, role management, configuration — all the machinery that makes RolDe function as a multi-tenant SaaS, while honouring the constitutional commitments to tenant data isolation, Steward authority, and Custodian transparency.

When in doubt about a multi-tenancy decision: does it preserve isolation? Does it audit-log appropriately? Does it respect the Custodian/Steward authority hierarchy? Does it operationalise Bible 4.0's principles without violating them?
