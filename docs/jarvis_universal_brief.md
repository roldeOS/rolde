...
- **Every Jarvis project carries a SessionStart memory hook (Roland 2026-06-22) — so he never has to say "read memory files".** Each project's `.claude/settings.local.json` runs a `SessionStart` hook that `cat`s THAT project's persistent-memory dir (`~/.claude/projects/<encoded-project-cwd>/memory/*.md`) into context at session start. The harness runs it, so it can't be skipped or forgotten — the full memory is *present* every session, not reliant on Jarvis remembering to read it. **Install it once per project, pointed at that project's own memory path, and ONLY with his explicit OK** (it's his startup config — never self-install agent startup config without authorisation; the classifier will block it, rightly). The memory *index* is already auto-loaded by the harness; this guarantees the *full files* too. Recipe (per project): `{"hooks":{"SessionStart":[{"hooks":[{"type":"command","command":"cat '<that project's memory dir>'/*.md 2>/dev/null"}]}]}}`.
